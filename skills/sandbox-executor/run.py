...
def run_in_docker(command: str, image: str = DOCKER_IMAGE,
...
def run_sandboxed(command: str, image: str = DOCKER_IMAGE,
                  timeout: int = SANDBOX_TIMEOUT,
                  mount: Optional[str] = None,
                  network: bool = False,
                  force_native: bool = False) -> dict:
    ...
    if not force_native and docker_available():
        ...
        if check.returncode != 0:
            ...
            if pull.returncode != 0:
                print(f"WARNING: Failed to pull image, falling back to native: {pull.stderr[:200]}",
                      file=sys.stderr)
